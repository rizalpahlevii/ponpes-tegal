...
switch ($aks) {
	case 'nilai':
		...
		<?php
			$nis = $_GET['nis'];
			$qisi = "SELECT s.nis,nama,asalsekolah,tmplahir,tgllahir,DAY(tgllahir) as tgl,MONTH(tgllahir) as bln,YEAR(tgllahir) as thn,s.id,s.statusmutasi,s.alumni,s.nisn,k.tingkat, k.kelas,k.idtahunajaran, t.tahunajaran 
														FROM siswa as s         
                                                        RIGHT JOIN `history` as h ON s.nis = h.nis
                                                        left join kelas k on h.idkelas = k.id
                                                        left join tahunajaran t on t.id = k.idtahunajaran 
                    where h.`nis` = '$nis' and h.id='idkelas' and t.id='idtahunajaran'
			";
