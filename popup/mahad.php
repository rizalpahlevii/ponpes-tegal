<?php
	...
	$sqltrans = mysqli_query($conn,"SELECT * FROM kemaarifan WHERE id = '$_GET[id]'") or die(mysqli_error());
				...
				$total = $tra['nominal'] - $tra['potongan'];
?>
<div class="w3-row">
...
<center><font>KWITANSI PEMBAYARAN SPP</font></center>
<center><font style="font-size:10px"> <?php echo tglindo(date('Y-m-d')) ?></font></center>
...
<?php
	...
	echo"<table class='w3-table w3-tiny w3-hoverable w3-bordered tbl' cellpadding='0'>
		...
		echo"<tr>
			<td>".tglindo($tra['date'])."</td>
			<td>".getBulanHijriah($tra['bulanke'])."</td>
			<td>Rp. ".number_format($tra['nominal'],0)."</td>
			<td>Rp. ".number_format($tra['potongan'],0)."</td>
