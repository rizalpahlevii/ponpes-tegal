...
switch ($aks) {
	case 'nilai':
		...
		<?php
			...
			$qisi = "SELECT a.`id`,a.`idpelajaran`,b.nama as pelajaran,a.`dasarpenilaian`, c.`keterangan` as dasarpenilaian, a.`idjenisujian`,d.`jenisujian`
			FROM `aturannhb` AS a
			               ...
			               <div class="profile-desk">
			               	...
			               	<table class="table table-borderless" >
			               		<tr>
			               			<td>Pelajaran</td>
			               			<td>:</td>
			               			<td><?php echo $isi['pelajaran']?></td>
			               		</tr>
			               		<tr>
			               			<td>Aspek Penilaian</td>
			               			<td>:</td>
			               			<td><?php echo $isi['dasarpenilaian']?></td>
			               		</tr>
			               		<tr>               			
			               			<td>Jenis Penilaian</td>
			               			<td>:</td>
			               			<td><?php echo $isi['jenisujian']?></td>
			               		</tr>
			               	</table>                   
			               </div>
			           </div>
	...
	default:
		...
		$idaturan = $_GET['idaturan'];
			...
			$qrt = "SELECT SUM(a.bobot) as bobotPK, COUNT(a.id) 
				 ...
				 WHERE a.nipguru = '$_GET[nip]' AND k.id = '$idkelas' 
			        	 		...
			        	 		<tr>
			        	 			...
			        	 			<?php
							      	$qu = mysqli_query($conn,"SELECT DISTINCT a.`bobot`, b.`kode`, b.`jenisujian`, a.id 
															...
															WHERE a.`idpelajaran` = '$isi[idpelajaran]' AND a.`nipguru` = '$_GET[nip]' AND a.`dasarpenilaian` = '$isi[iddasarpenilaian]' AND b.id = a.idjenisujian ORDER BY b.jenisujian");
							      	$i=1;

							      	while($u = mysqli_fetch_assoc($qu)){
							      	?>
							      	<th class="text-center" >
							      		<font class="popovers" data-original-title="BOBOT" data-content="<?php echo $u['bobot'] ?>" data-placement="top" data-trigger="hover" ><?php echo $u['jenisujian'] ?></font> 
			        	 	...
			        	 	<tbody>
							    ...
							    <tr>
							        ...
							        <td align="center">
		                                ...
		                                JOIN `rpp` as r on q.`idrpp` = r.`id`
										JOIN `statusguru` as d on d.`id` = a.`statusguru` 
										WHERE a.`nip` = '$_SESSION[id_user]' AND q.`idkelas` = '$idkelas' AND q.`idsemester` = '$idsemester' AND q.`idjenis` = '$u[id]' AND dp.`id` = '$idaturan'") or die(mysqli_error());
										$cekq = mysqli_num_rows($sqltrans);
										if ($cekq !== 0) {
											$i=1;
											$sumq=0;
