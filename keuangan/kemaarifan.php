<?php
	...
	switch ($act) {
		...
		if(!empty($_GET['id']))
			{
				$act = "$aksi?mod=kemaarifan&act=edit";
				$query = mysqli_query($conn,"SELECT * FROM kemaarifan WHERE id = '$_GET[id]'");
				...
				else
				{
					if($_SESSION['level']=='keuangan'){
						header("location:med2.php?mod=kemaarifan");
					}elseif ($_SESSION['level']=='admin') {
						header("location:med.php?mod=kemaarifan");
					}
			...
			else
			{
					...
					$nis = $_GET['nis'];
					$thn = $_GET['thn'];
				$act = "$aksi?mod=kemaarifan&act=simpan";
			}
			        ...
			        <!-- page start-->
				                    ...
				                    <div class="panel-body">
				                        <div class="position-center">
				                            ...
				                            <div class="form-group">
				                                ...
				                                <div class="col-lg-10">
			                                          <select name="bulan" class="form-control round-input" style="width: 100%">
			                                ...
			                                <div class="form-group">
				                                ...
				                                <div class="col-lg-10">
				                                    <input type="text" id="harga" name="nominal" class="form-control round-input currency4" value="<?php echo isset($c['nominal']) ? $c['nominal'] : '' ;?>" required>
				                                </div>
				                            </div>
                                            <div class="form-group">
				                                <label for="inputEmail1" class="col-lg-2 col-sm-2 control-label">Potongan (Rp.)</label>
				                                <div class="col-lg-10">
				                                    <input type="text" id="potongan" name="potongan" class="form-control round-input currency4" value="<?php echo isset($c['potongan']) ? $c['potongan'] : '' ;?>" required>
				                                </div>
				                            </div>
				                            <div class="form-group">
				                                <label for="inputEmail1" class="col-lg-2 col-sm-2 control-label">Total (Rp.)</label>
				                                <div class="col-lg-10">
				                                    <input type="text" id="total" name="total" class="form-control round-input currency4" value="" readonly>
				                                </div>
				                            </div>
                        ...
                        <script type="text/javascript">
                            $(document).ready(function() {
                                $("#potongan, #harga").keyup(function() {
                                    var harga =($('#harga').val() != '' ? $('#harga').val() : 0);
									var sb = harga.replace(/[.](?=.*?\.)/g, '');
									var outsb = parseFloat(sb.replace(/[^0-9.]/g,''));
									
									var potongan =($('#potongan').val() != '' ? $('#potongan').val() : 0);
									var pt = potongan.replace(/[.](?=.*?\.)/g, '');
									var outpt = parseFloat(pt.replace(/[^0-9.]/g,''));
									
									
                                    var total = parseInt(outsb) - parseInt(outpt);
                                    $("#total").val(total);
                                });
                            });
                        </script>
				            ...
				            <div class="col-sm-12">
				                <section class="panel">
				                    <header class="panel-heading">
				                        ...
				                        <span class="tools pull-right">
											...
											<button class="btn btn-primary btn-xs">
												Tambah Pembayaran Mahad <i class="fa fa-plus"></i>
											</button>
											</a>
				                         </span>
				                    </header>
				                    <div class="panel-body table-responsive">
		                                ...
		                                <tbody>
		                                <?php
											$query = "SELECT `id`, `nis`, `idtahunajaran`, `bulanke`, `nominal`,`potongan`, DAY(date) AS tanggal, MONTH(date) AS bulan, YEAR(date) AS tahun FROM `kemaarifan` WHERE `nis` = '$tra[nis]' AND `idtahunajaran` = '$tra[idtahunajaran]' order by `bulanke`";
		                                ...
		                                <tr class="">
		                                   ...
		                                   --> 
		                                        <a target="_blank" href="popup/popup.php?mod=mahad&id=<?php echo $m['id'] ?>" ><button class="btn btn-warning btn-sm"><i class="fa fa-print"></i></button> </a>
		                                        <a href="<?php echo $linkaksi ?>&act=aksi&id=<?php echo $m['id'] ?>" ><button class="btn btn-success btn-sm"><i class="fa fa-edit"></i></button> </a>
												<a href="<?php echo $aksi ?>?mod=kemaarifan&act=hapus&id=<?php echo $m['id'] ?>" onclick="return myFunction();"><button class="btn btn-danger btn-sm"><i class="fa fa-times"></i></button> </a>
