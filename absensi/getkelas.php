...
$sqll = "SELECT * 
		...
		WHERE `idkelas` = '$q' ORDER BY a.`nama` ASC" ;
    	...
    	<div class="adv-table editable-table table-responsive">
            ...
            <table class="table table-striped table-hover table-bordered" id="example">
                ...
                <tr class="">
                    ...
                    <?php
                    $sqla = "SELECT * FROM `siswa_nonaktif` WHERE `nis` = '$datal[nis]'";
                        $qakt = mysqli_query($conn, $sqla);
                        if(mysqli_num_rows($qakt) > 0){
                            $akt = mysqli_fetch_assoc($qakt);
                        ?>
                        <td align="center">                            
                            <button type="button" class="btn btn-danger btn-block">Nonaktif</button>
                            <input type="hidden" name="kehadiran[]" value="Nonaktif">
                        </td>
                        <?php
                        }else{
                        ?>                        
